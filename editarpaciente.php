...
<body>
    ...
    $sql="Select * from tbpacientes where id=$id";
    $result = $conn->query($sql);
    $dados = $result->fetchAll(PDO::FETCH_ASSOC);
    foreach ($dados as $linha) {
    ?>

    <form name="form1" action="editarpaciente2.php" method="post">
        ...
        <label>Data de Nascimento:</label><input type="date" name="datanascimento" 
            value="<?php echo $linha["datanascimento"]; ?>"><br>
        <label>Convênio:</label><input type="text" name="convenio" 
            value="<?php echo $linha["convenio"]; ?>" placeholder="Digite o seu convênio"><br>
